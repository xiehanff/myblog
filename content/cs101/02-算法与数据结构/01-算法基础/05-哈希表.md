...
> **关键认知：** 哈希表用顺序能力换取等值操作的开销：它不做 min/max、前驱后继或范围查询，换来的是在合理假设下期望常数的 `put`/`get`/`remove`。
...
还有一个容易踩的坑：只重写 `==` 而忘记 `hashCode`。Dart 官方明确要求"如果子类重写了 `operator ==`，就应该同时重写 `hashCode`"，否则它不能作为默认 `Set` 和 `Map` 的键。实测中更隐蔽的是 `const` 对象：两个 `const BrokenPoint(1, 2)` 会被规范化为同一个实例，查找"碰巧"成功，缺陷被掩盖；换成运行时构造的等值对象后，100 对等值键里能互相查到的有 0 对。

> **关键认知：** 哈希值只负责把比较范围缩小到一个桶，键是否相等始终由 `==` 决定；`hashCode` 与 `==` 必须表达同一套等价关系。
...
> **关键认知：** 哈希表的正确性不依赖"避免冲突"，而依赖"能分辨冲突的键"：冲突交给 `==` 处理，哈希函数只需要让冲突尽量少。
...
> **关键认知：** 负载因子决定的是期望成本，而不是常数保证；"扩容摊还到 O(1)"的前提是桶数至少按固定倍数增长。
...
> **关键认知：** 开放寻址把所有键放在同一块连续内存里，用缓存局部性换取更小的空间开销；代价是容量必须留空，删除必须留下"非空"的痕迹。
...
Dart 这边能确认的是公开契约，而不是内部结构：官方文档明确 `Object.hash` 系列不保证跨运行稳定，本次验证中同一程序两次运行的 `Object.hashAll([1, 2, 3])` 取值确实不同，所以不要把哈希值持久化或写进协议；Dart 内部用什么探测方式、初始桶数与扩容阈值都没有公开承诺，本文也不做断言。
...
> **关键认知：** 期望 O(1) 是对"哈希分布良好、负载因子受控"这两个前提的结论；去掉前提，哈希表的最坏成本是 O(n)，而且可以被外部输入故意触发。
...
> **关键认知：** 说"Dart 的 Map 无序"要分两层：它不按键排序，但默认实现（`LinkedHashMap`）**按插入顺序迭代**；只有 `HashMap` 才不承诺顺序。
...
<figure class="diagram-scroll"><img src="./05-哈希表.assets/hash-table-lru-links.svg" alt="LRU 缓存中键到节点以及双向链表的关系"></figure>

链表节点的链接与断开细节见[链表](./03-链表.md)：那里只概述"哈希 + 双链表"，不重复哈希定位的机制。

> **关键认知：** 哈希表只提供"按键定位"这一种能力；一旦需求里出现顺序、范围或优先级，就必须叠加另一种结构，而不是指望哈希表自己解决。
...
> **关键认知：** 选哈希表还是有序映射，取决于"需要哪些操作"，而不是"哪个平均更快"；只要出现一个有序操作，哈希表就需要叠加或替换结构。
