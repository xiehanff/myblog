# 算法与数据结构｜05a-Dart HashMap 的实现：从哈希表原理到 Dart SDK 源码
...
```mermaid
flowchart LR
  subgraph buckets["_buckets（初始 8 桶）"]
    b0["[0] null"]
    b1["[1] 17 & 7 = 1"]
    b2["[2] 10 & 7 = 2"]
    b37["[3–7] …"]
  end
  b1 --> A["Entry A<br/>key=A · value=1 · hash=17<br/>next = null"]
  b2 --> B["Entry B<br/>key=B · value=2 · hash=10<br/>next 指向后继节点"]
  B --> C["Entry(…)"]
```
...
```mermaid
flowchart TB
  subgraph data["_data：按插入顺序存放键值"]
    direction LR
    d0["key0 / value0"] --> d1["key1 / value1"] --> d2["key2 / value2"] --> d3["…"]
  end
  subgraph index["_index：开放寻址查找"]
    direction LR
    i0["探测信息"] --> i1["探测信息"] --> i2["探测信息"] --> i3["探测信息"]
  end
  index -->|按哈希探测，定位 _data 中的键值对| data
```
...
```mermaid
flowchart TB
  L1["01 · 公开契约<br/>dart:core / dart:collection<br/>公开 API、抽象接口、文档契约"]
  L2["02 · 实现绑定<br/>运行时 patch<br/>按 VM、Wasm、JavaScript 等后端绑定实现"]
  L3["03 · 存储结构<br/>具体数据结构<br/>链地址法、紧凑开放寻址、JavaScript 对象 / Map 等"]
  L4["04 · 执行优化<br/>编译器和运行时优化"]
  L1 --> L2 --> L3 --> L4
```
