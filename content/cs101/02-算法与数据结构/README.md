...
面试中常见的落差是：题目能用背过的模板写出来，但被追问“复杂度是怎么得到的”“换一个约束还成立吗”“为什么这里必须用哈希表”时讲不清。题解库能回答“这道题怎么写”，却回答不了“遇到没见过的问题该从哪里想起”。

本板块因此按“知识点主线优先、题解归档为辅”组织。15 篇知识点文章负责建立模型、推导性质、给出边界与示例；`02-LeetCode题解/` 与 `03-剑指Offer题解/` 只作为历史归档，用于对照与查证，不承担教学职责。主线示例统一使用纯 Dart 并经过实际运行验证；归档正文保留既有 Java 实现，并标注为历史材料。
...
- 主线示例为纯 Dart（不依赖第三方包，验证环境 3.12.2），采用 LeetCode 风格函数签名，不从标准输入读取；归档正文保留 Java，不做语言迁移。
...
```text
分析基础   01 复杂度分析与算法思想（全板块前置）
               │
数据组织   02 数组与字符串 ── 03 链表 ── 04 栈与队列 ── 05 哈希表
               │
组合结构   06 树与二叉树 ── 07 堆与优先队列
           08 图与并查集
               │
算法范式   09 排序 ── 10 查找与二分查找
           11 递归、分治与回溯 ── 12 动态规划 ── 13 贪心
               │
专项技巧   14 位运算技巧
           15 字符串算法
```
...
- [LeetCode 历史题解归档](./02-LeetCode题解/README.md)：16 个分类汇总，正文保留 Java，按知识点反向挂接。
- [剑指 Offer 历史题解归档](./03-剑指Offer题解/README.md)：76 篇题解，正文保留 Java，按题号与知识点双向索引。
- 用法：先读对应知识点主文，再用归档对照旧解法或核对题面；归档不提供概念定义，也不作为学习路线。
- 归档正文的题号与链接来自原平台，可能已经失效；具体说明与勘误记录见两个归档 README。

## 如何读一篇知识点文章

1. 动机：先读开篇“为什么需要”，把知识放进真实问题，而不是从定义开始背。
2. 模型与不变量：找到该结构或算法的判定条件，也就是文中的“关键认知”。
...
- 归档：Java 代码只存在于 `02-` 与 `03-` 两个归档目录，主线不复用归档代码。

## 维护与勘误

- 概念错误：说明文章、章节与期望结论，并附权威来源（教材章节或官方文档）。
- 代码问题：附 Dart 版本、最小复现与实际输出。
- 题解与链接：指出归档文件、失效题号或链接，尽量给出现行平台的对照。
...
> 算法板块以 15 篇知识点文章为唯一学习主线，题解只作为可对照、可查证的历史归档。
