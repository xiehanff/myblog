...
<figure class="diagram-scroll"><img src="./09-组合.assets/pattern-relationship.svg" alt="关系图：组合：统一处理叶子与容器"></figure>
...
- 与普通树结构的区别：组合模式的重点不是“能存子节点”，而是“叶子与容器共享同一协议并统一客户端调用”。只是用一个 `List` 装子节点、客户端依旧到处判断类型，那只是树，不是组合模式。
