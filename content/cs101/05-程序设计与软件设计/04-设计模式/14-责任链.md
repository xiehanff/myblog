...
```mermaid
flowchart TD
  Start[Client 发出请求] --> A{Handler A 能处理吗？}
  A -->|能| ResultA[Handler A 处理并返回结果]
  A -->|不能，交给 next| B{Handler B 能处理吗？}
  B -->|能| ResultB[Handler B 处理并返回结果]
  B -->|不能，交给 next| C{Handler C 能处理吗？}
  C -->|能| ResultC[Handler C 处理并返回结果]
  C -->|不能，链到末尾| None[没有处理者]
```
