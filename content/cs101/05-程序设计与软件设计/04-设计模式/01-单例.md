...
动机：配置、日志、会话这类资源天然应该只有一份。如果各处可以随手 `new`，就会出现多个对象各持一份状态、互相看不见的场面。但要抓住重点：单例真正要回答的是“谁来控制生命周期，以及谁有权拿到它”，“省内存”从来不是它的重点。GoF 的答案是私有构造器加全局访问点；Dart 提供了更直接的语言机制，也带来了新的边界。
...
依赖方向：客户端直接依赖具体类型。这正是单例常被诟病的原因——依赖被藏进了函数体，没有写在构造器签名上，替换和测试都要绕路。
...
如果实例依赖运行期才知道的外部配置（例如时区、环境），静态初始化无法完成注册，此时用显式初始化：没初始化就访问属于使用错误，应当立刻失败，不要返回半成品。
