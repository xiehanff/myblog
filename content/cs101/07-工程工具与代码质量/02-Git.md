...
这四个区域的差异在职责，把它们当成"四个备份文件夹"并不准确：
...
四个区域之间没有任何自动同步。你在工作区改了文件，暂存区不知道；你 fetch 了远程，工作区也不会变。所有推进都必须显式命令。
...
一旦对象写入仓库，它就不会被"修改"。后续所谓"改提交内容"，本质上是**新建一个新的对象，然后让分支指针改指向新对象**，并不存在原地修改。
...
Git 对象不可变，一切"修改历史"都是"新建对象 + 移动指针"。哈希变化是这一模型的直接结果，不是异常。
...
最常见的情况是 `HEAD -> main -> C3`：你当前在 `main` 分支上，`main` 分支当前指向提交 `C3`，`HEAD` 先指向 `main`，并不直接指向 `C3`。
...
提交、分支、合并、回退，本质上都只是"对象新增了"或"某个指针改指向了"。抓住"对象不动，指针在动"，大部分 Git 操作都能看懂。
...
`origin/main` 不是远程服务器上的那个分支本体，它只是你本地仓库里的一条远程跟踪引用。fetch 更新的是它，不是你的代码。
...
pull = fetch + merge（或 rebase）。没把这两层分开理解，就很容易在冲突、快进、merge、rebase 之间混淆。
...
分支不是"一份代码的拷贝"，只是一个指向某次提交的可移动引用。这就是分布式 Git 建分支廉价到可以随便建的根本原因。
...
fast forward 只动指针、不产生新提交；`--no-ff` 强制产生一个 merge commit，把"这里有过一次合并"记进历史。团队通常在合入长期分支时统一要求 `--no-ff`，保留分支语义。
...
stash 不是神奇缓存区——它本质上也是提交对象，只是挂在特殊引用（refs/stash）上。stash 能保存、列出、恢复，用的还是对象和引用这一套。
...
`restore` 类动作的核心是**用某个已知快照覆盖工作区或 Index**，并不改历史：
...
revert 和 reset 的选择标准只有一条——这段历史是否已经共享给别人。已推送的用 revert（追加反向提交，不改历史），未推送的本地提交才可以用 reset 整理。
...
push 被拒绝的原因是你准备推的历史不能直接接在远程分支后面——Git 在阻止你覆盖别人的提交，跟"网络问题"无关。
...
`rebase` 要把"你这条分支自己新增的那几次提交"摘下来，换个基底重新播放一遍，并不会汇合两段历史。本质动作是：
...
原来的那串提交被新提交替换了位置，并没有被"修改"——哈希变化就是这个原因。所以：
...
很多冲突的根源在需求和设计没有提前收敛——冲突标记落在几行文本上，真实冲突点常常是业务决策冲突。
...
所以 rebase 时经常出现"文件明明已经是我想要的样子了，Git 还是停下来"——它要判断的是"这一笔旧提交在新上下文里还能不能成立"，跟最终文件长什么样无关。
...
轻量标签和附注标签的区别在于是否携带元数据；发版应使用附注标签。另外 `git push` 默认不推标签，必须显式推。
...
回滚类操作（reset/rebase/删分支）只移动指针，提交对象通常仍在版本库里；误操作后用 `git reflog` 找到目标位置，再用 `git reset --hard HEAD@{n}` 回去，或 `git branch <名字> HEAD@{n}` 恢复被删分支。reflog 的默认保留期受引用是否可达、仓库配置和垃圾回收影响，常见的可达记录约为 90 天、不可达记录约为 30 天，不应把它当永久备份。
...
`.gitignore` 只对**未跟踪**文件生效。一个文件已经被 Git 跟踪后再加进 `.gitignore`，它的修改仍会被记录；需要先 `git rm --cached <file>` 把它移出暂存区。密码、令牌等秘密一旦提交过，即使后来忽略并删除，也仍可能留在历史中，应立即吊销并按凭据泄露流程处理。
