...
**原因**：说白了，第三方库内部怎么实现，随时可能随版本变。Mock 它，等于你把它的内部当成了稳定的东西，这个假设随时可能失效。自己封一层接口出来，Mock 自己这层。
...
BDD 的价值不在写法本身，**让 PM、QA 这些非技术角色也能参与测试用例评审**，这才是它真正的好处。
...
**关键认知**：100% 覆盖率不等于 100% 正确。覆盖率只告诉你"这行代码被执行过"，不告诉你"边界条件是不是都验证了"。
