...
这也是为什么平台通道不是“直接调 Java/OC 方法”，而是“把一个二进制消息交给另一侧处理，再等结果回来”。
...
`PlatformView` 的本质不是简单把原生控件塞进来，而是把**原生视图树**放进 Flutter 的**合成链路**里。
...
这个 id 不是全局永久 id，而是和当前 Flutter view 的生命周期绑定的。
