...
这个现象是 Flutter 的**约束驱动（Constraint-Driven）** 布局模型带来的必然结果，并不是 “bug”。本文将从底层原理出发，彻底拆解这个现象的本质，帮你彻底掌握 Flutter 布局的核心逻辑。
...
当`minWidth == maxWidth` 且 `minHeight == maxHeight`时，该约束为紧约束。直白地说，父组件给子组件下达了 “死命令”—— 你必须刚好长这么大，没有任何选择空间。此时子组件设置的任何宽高都将失效，只能严格按照父级给定的固定尺寸渲染。
...
当`minWidth == 0` 且 `minHeight == 0`，仅限制最大宽高时，该约束为松约束。同样直白地说，父组件给子组件划定了 “上限”—— 你可以长到任意大小，但不能超过我给的最大尺寸，最小可以是 0。此时子组件可以自由决定自身的尺寸，只要不超过父级给定的最大值，自身设置的宽高可以正常生效。
...
约束的计算与传递严格遵循 Flutter 的渲染流水线，在固定阶段统一调度执行，并不是随机发生的。
