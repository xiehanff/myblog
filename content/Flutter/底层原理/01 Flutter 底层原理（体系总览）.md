...
> 下面把“Flutter 底层原理”和“Flutter 应用启动到交互的完整流程”放在同一条链路上讲清楚：
...
**普通 `key` 解决局部匹配，`GlobalKey` 解决跨树移动**。
...
**Flutter 的直接操作对象是配置，屏幕像素要等框架在帧调度下走完树结构更新和渲染流水线才产生。**
...
Flutter 的 Dart 代码运行在 UI isolate 里。
...
用户点击、滑动、按键先进入事件分发链路，不会直接落到某个 Widget 上。
...
这说明 Flutter 的更新模型是“状态变化驱动整条渲染链路”，不做命令式的 DOM 改写。
...
资源加载这一块很关键，因为很多启动卡顿的根源在资源，不在 build。
