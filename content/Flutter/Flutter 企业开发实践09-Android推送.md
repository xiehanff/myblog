...
> 在国内 Android 生态里，推送不是"加个 SDK 就能用"的事。厂商通道常被当成救命稻草，可它到底是不是必选项？这篇我分五块讲：推送怎么选型、厂商通道怎么接、消息类型怎么设计、通知点击怎么路由，还有 Flutter 侧怎么封装。同时拿一个上线半年的 Flutter 混合开发项目做例子（下文简称"该项目"）：它 Android 只走极光长连接，一家厂商通道都没接，这个取舍正好可以当"要不要厂商通道"的样本。
